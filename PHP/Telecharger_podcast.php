...
if (isset($_POST['submit'])) {
    ...
    $podcast_file = $_FILES['podcast_file']['name']; //name — Le nom original du fichier, comme sur le disque du visiteur (exemple : mon_icône.png).
    ...
    $podcast_file_size = $_FILES['podcast_file']['size']; //size — La taille en octets du fichier télécharger.
    $podcast_file_error = $_FILES['podcast_file']['error']; //error — Le code d'erreur, qui permet de savoir si le fichier a bien été télécharger.
    ...
    $allowed = array('mp3', 'wav', 'ogg'); //array — Crée un tableau, avec les valeurs passées en paramètres, comme éléments qui représente les extensions autorisées
    
    if (in_array($podcast_file_actual_ext, $allowed)) {
        if ($podcast_file_error === 0) { //0 = pas d'erreur
            if ($podcast_file_size < 10000000) { //10MB
                $podcast_file_name_new = uniqid('', true) . "." . $podcast_file_actual_ext; //uniqid — Génère un identifiant unique basé sur l'heure courante en microsecondes et sur un paramètre binaire optionnel, qui permet de le rendre encore plus unique parce que est important que le nom du fichier soit unique
...
<textarea name="interview" id="interview" class="form-control"></textarea>
</div>
