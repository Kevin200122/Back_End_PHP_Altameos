    ...
    if (in_array($album_file_actual_ext, $allowed)) {
        if ($album_file_error === 0) {
            if ($album_file_size < 10000000) {
                ...
                $req = $conn->prepare('INSERT INTO albums (title, file_path, rubrique, emission, interview, id_categorie) VALUES (?, ?, ?, ?, ?, ?)');
                $req->execute([$album_title, $album_file_destination, $rubrique, $emission, $interview, $id_categorie]);
...
<div class="mb-3">
<label for="album_fichier" class="form-label">album File:</label>
<input type="file" name="album_fichier" id="album_fichier" class="form-control" required>
</div>
<div class="mb-3">
...
<textarea name="interview" id="interview" class="form-control" required></textarea>
</div>
<div class="mb-3">
